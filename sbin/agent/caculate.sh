...
if [ ! -f $1 ];then
	...
	lines=`cat $tmpraw | grep "^ [0-9a-zA-Z-]\{20\}"   | grep -v geoip | grep -v device_name | awk '{print $1}' | uniq | wc -l`
	j=0
	for i in `cat $tmpraw | grep "^ [0-9a-zA-Z-]\{20\}"   | grep -v geoip | grep -v device_name | awk '{print $1}' | uniq` ;do
       		((j++))
        	cat $tmpraw |\
        	grep $i |  \
